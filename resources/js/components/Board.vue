<template>
    <div
        class="board"
        @click.right.prevent="toggleNewShipOrientation"
        :class="[modality, state]"
    >
        <div class="container position-relative" @click="placeNewShip">
            <img
                v-for="attack in attacks"
                v-if="attack.hit"
                src="/images/attack-indicator.png"
                :width="cellHeight"
                :height="cellHeight"
                :style="{
                    position: 'absolute',
                    top: (attack.target_y - 1) * cellHeight + 'px',
                    left: (attack.target_x - 1) * cellHeight + 'px',
                    zIndex: 9999,
                }"
            />
            <ship
            ...
            <ship
                ...
                :orientation="newShip.orientation"
                :length="newShip.length"
                key="new-ship"
                :class="{ 'invalid-position': !isValidNewShipPosition }"
            />
...
export default {
    ...
    methods: {
        onCellClick(x, y) {
            switch (this.state) {
                case "placing":
                    ...
                    break;
                case "attacking":
                    if (this.modality == "enemy") this.sendAttack(x, y);
                    break;
            }
        },
        sendAttack(x, y) {
            if (this.modality != "enemy" && this.state != "attacking") return;
        ...
        cellClass(x, y) {
            let attack = _.find(this.attacks, {
                target_x: Number(x),
                target_y: Number(y),
            });
            let result = "cell";
        ...
        toggleNewShipOrientation() {
            if (this.state != "placing" || this.modality != "own") return;
            if ("h" == this.newShip.orientation) this.newShip.orientation = "v";
            else this.newShip.orientation = "h";
        },
        moveNewShip(x, y) {
            if (this.state != "placing" || this.modality != "own") return;
        ...
        placeNewShip() {
            ...
            if (!this.isValidNewShipPosition) return;
        ...
        shipsIntersect(shipA, shipB) {
            if (shipA.orientation === "h") {
                if (shipB.orientation === "h")
                    return (
                        shipA.position.y == shipB.position.y &&
                        shipA.position.x >= shipB.position.x &&
                        shipA.position.x < shipB.position.x + shipB.length
                    );
                if (shipB.orientation === "v")
                    return (
                        shipA.position.y >= shipB.position.y &&
                        shipA.position.y < shipB.position.y + shipB.length &&
                        shipA.position.x + shipA.length - 1 >=
                            shipB.position.x &&
                        shipA.position.x <= shipB.position.x
                    );
            }
            ...
            if (shipA.orientation === "v") {
                ...
                if (shipB.orientation === "h")
                    return (
                        shipA.position.x >= shipB.position.x &&
                        shipA.position.x < shipB.position.x + shipB.length &&
                        shipA.position.y + shipA.length - 1 >=
                            shipB.position.y &&
                        shipA.position.y <= shipB.position.y
                    );
            }
        ...
        isValidNewShipPosition() {
            ...
            if (
                this.ships.some((ship) =>
                    this.shipsIntersect(ship, this.newShip)
                )
            )
                return false;
